...
def refine_transcript_with_ai(model, raw_transcript):
    """
    تحسين وتصحيح النسخ الصوتية باستخدام الذكاء الاصطناعي
    يقوم بـ:
    1. تصحيح الأخطاء الإملائية والنحوية
    2. إعادة صياغة الجمل غير المفهومة
    3. إضافة علامات الترقيم المناسبة
    4. الحفاظ على المعنى الأصلي والسياق
    """
    logger.info("Starting AI-powered transcript refinement...")
    
    refinement_prompt = f"""أنت متخصص في تحسين النصوص العربية. لديك نص تم استخراجه من تسجيل صوتي قد يحتوي على أخطاء إملائية أو نحوية.

المطلوب منك:
1. تصحيح جميع الأخطاء الإملائية والنحوية
2. إضافة علامات الترقيم المناسبة (نقاط، فواصل، علامات استفهام، إلخ)
3. إعادة صياغة الجمل غير المفهومة أو الملتبسة بطريقة واضحة
4. الحفاظ على المعنى الأصلي والسياق بالكامل
5. عدم حذف أي معلومات مهمة
6. تحسين التدفق والقراءة دون تغيير المحتوى الأساسي

النص الأصلي:
"{raw_transcript}"

أرجع النص المصحح والمحسّن فقط، بدون أي تعليقات إضافية."""

    try:
        refined_text = analyze_with_gemini_with_retry(model, refinement_prompt)
        logger.info("Transcript refinement completed successfully")
        return refined_text
    except Exception as e:
        logger.warning(f"Transcript refinement failed: {e}, returning original transcript")
...
async def analyze_audio(file: UploadFile = File(...), meeting_id: str = None):
    """
    تحليل الملف الصوتي واستخراج النسخ الصوتية والتحليلات مع التصحيح الذكي
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Raw transcription complete: {raw_transcript[:100]}...")
        ...
        if api_key:
            try:
                ...
                # تطبيق التصحيح الذكي
                refined_transcript = refine_transcript_with_ai(model, raw_transcript)
                logger.info(f"Refined transcription: {refined_transcript[:100]}...")
            except Exception as e:
                logger.warning(f"AI refinement failed, using raw transcript: {e}")
                refined_transcript = raw_transcript
        else:
            logger.warning("GEMINI_API_KEY not set, skipping AI refinement")
...
Only return the JSON. No extra text.
                ...
                try:
                    ...
                    analysis = {
                        "summary": refined_transcript[:500] if len(refined_transcript) > 500 else refined_transcript,
                        "notes": [],
                        "todo_list": [],
                        "key_decisions": [],
                        "action_items": []
                    }
                    formatted_transcript = [{"speaker": "متحدث 1", "text": refined_transcript}]
                    speakers = ["متحدث 1"]
                    
            except Exception as e:
                logger.warning(f"Warning: Gemini analysis failed: {e}")
                # إذا فشل التحليل، استخدم النص المصحح كبديل
                analysis = {
                    "summary": refined_transcript[:500] if len(refined_transcript) > 500 else refined_transcript,
                    "notes": ["تم استخراج النص من الصوت بنجاح وتصحيحه"],
                    "todo_list": [],
                    "key_decisions": [],
                    "action_items": []
                }
        ...
        result = {
            ...
            "raw_transcript": raw_transcript,  # النسخ الأصلية
            "transcript": refined_transcript,   # النسخ المصححة والمحسّنة
            "speakers": speakers,
